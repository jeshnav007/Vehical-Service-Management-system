...
const ServiceCenterDashboard = () => {
  ...
  useEffect(() => {
    ...
    // Fetch technicians directly from Firestore client SDK.
    // This bypasses the backend entirely, avoiding token/role issues on deployed envs.
    const fetchTechnicians = async () => {
      try {
        ...
        const techs = snapshot.docs
          ...
          .filter(t => t.isActive !== false); // exclude deactivated techs
        setTechnicians(techs);
      } catch (error) {
        console.error('Failed to fetch technicians from Firestore:', error.message);
